            ...
            for model_type in [['TRA', 'COR']]:
                for score_sum in [True, False]:
                    for fold in range(5):
                        ...
                        mid = 'Total' if score_sum else 'PerLocation'
                        sufix = 'Sum.onnx' if score_sum else '.onnx'
                        model_path:str = str(model_path / mid / f'{fea}__{site}_{mt}_fold{fold}{sufix}')
                        src:str = model_path.replace('onnx_model', 'raw_model').replace('.onnx', '.model')
