...
class UNet5(nn.Module):
    def __init__(self, n_channels, n_classes, 
        ...
        self.down4 = (PreDown(128*w, 256*w))  # x4  [b, 128*, l/8, w/8] -> [b, 128*, l/16, w/16] -> x5 [b, 128*, l/16, w/16]
        factor = 2 if bilinear else 1
        self.down5 = (PreDown(256*w, 256*w//factor))
        self.bottom = (DoubleConv(256*w // factor, 512*w // factor))  # x5 [b, 128*, l/16, w/16] --> x5 [b, 256*, l/16, w/16]
        self.up0 = DualUp(512*w // factor, 256*w//factor, bilinear)
    ...
    def forward(self, x):
        ...
        x6 = self.down5(x5)
        x6 = self.bottom(x6)  # x5 [b, 128*, l/16, w/16] -> [b, 256*, l/16, w/16]
        x = self.up0(x6, x5)
        x = self.up1(x, x4)  
        # x5 [b, 256*, l/16, w/16] --> x5 [b, 128*, l/8, w/8]
        # x5 + x4 -- [b, 2x128*, 1/8, w/8]  --> x [b, 128*, l/8, w/8]
