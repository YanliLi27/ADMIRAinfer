...
def get_id_from_mri_pre(mri_root:str, groups:list=['TRT', 'PLA'], sites:list=['Wrist', 'MCP', 'Foot'], views:list=['TRA', 'COR']) -> pd.DataFrame:
    data = {}
    ...
    for patient in tqdm(id_list):
        if patient.split('_')[-1] not in groups: 
            continue
        # \20150319
        ...
        for date in dates:
            ...
            for site in sites:
                for view in views:
                    matches = [item for item in all_scan_folder if re.search(f'{site}_Pre{view}T1f', item)]
                    if matches:
...
def te_pre_initialization(mri_root:str=r'R:\\AIMIRA\\AIMIRA_Database\\LUMC', 
    ...
    if not os.path.exists(r'./datasets/te_pre_mri_init.csv'): 
        mri_id_path:pd.DataFrame = get_id_from_mri_pre(mri_root)
        mri_id_path.to_csv(r'./datasets/te_mri_init.csv')
    else:
        mri_id_path = pd.read_csv(r'./datasets/te_pre_mri_init.csv')
    # ID (Csa003), DATE(20202020), ID_DATE(ID;DATE), Site_View * 6 (abs_path;NtoN+7)
    if not os.path.exists(r'./datasets/te_pre_ramris_init.csv'): 
        ramris_id_score:pd.DataFrame = get_id_from_ramris(ramris_root)
        ramris_id_score.to_csv(r'./datasets/te_pre_ramris_init.csv')
    else:
        ramris_id_score = pd.read_csv(r'./datasets/te_pre_ramris_init.csv')
...
def getdata_pre(task:Literal['TE'], site:Literal['Wrist','MCP','Foot'], feature:Literal['TSY','SYN','BME'], view:list[str]=['TRA', 'COR'], 
                filt:Optional[list]=None, score_sum:bool=False, path_flag:bool=True):
    path_default = {'TE':r'./datasets/te_pre_init.csv'}
    paths = path_default[task]

    if not os.path.exists(paths):
        df:pd.DataFrame = te_pre_initialization()
        df.to_csv(paths)
    else:
