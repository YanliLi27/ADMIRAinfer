...
from .csv_utils import Transformer


def conv_nxn_bn_group3d(inp: int, oup: int, kernal_size: Union[int, tuple] = 3, stride: Union[int, tuple] = 1,
                        groups: int = 4):
...
class NormCNN3d(nn.Module):
    def __init__(self, inp, oup, kernal_size=3, stride=1, groups=4):
        ...
        self.cnn = nn.Sequential(
            nn.Conv3d(inp, oup, (1, kernal_size, kernal_size), (1, stride, stride), (0, 1, 1), groups=groups,
                      bias=False),
            nn.BatchNorm3d(oup),
            nn.SiLU()
        )
...
class DSConvBlock3d(nn.Module):
    def __init__(self, inp, oup, stride=1, groups: int = 4, expansion=4):
        ...
        self.conv = nn.Sequential(
            ...
            nn.SiLU(),
            # dw
            nn.Conv3d(hidden_dim, hidden_dim, (1, 3, 3), (1, stride, stride), 1, groups=groups * hidden_dim,
                      bias=False),
            nn.BatchNorm3d(hidden_dim),
            nn.SiLU(),
            # pw-linear
...
class NormalConvBlock3d(nn.Module):
    ...
    * if oup is None, the oup== inp * expansion
    ...
    def __init__(self, inp, oup, kernal_size=3, stride=1, groups: int = 4, expansion: int = 0):
        ...
        if expansion == 0:
            self.conv = nn.Sequential(
                nn.Conv3d(inp, oup, (1, kernal_size, kernal_size), (1, stride, stride), (0, 1, 1), bias=False,
                          groups=groups),
                nn.BatchNorm3d(oup),
                nn.SiLU(),
            )
            ...
            self.conv = nn.Sequential(
                ...
                nn.SiLU(),
                # dw
                nn.Conv3d(hidden_dim, hidden_dim, (1, 3, 3), (1, stride, stride), (0, 1, 1), groups=hidden_dim,
                          bias=False),
                nn.BatchNorm3d(hidden_dim),
                nn.SiLU(),
                # pw-linear
...
class ViTBlock3d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = rearrange(x, 'b (ph pw) (h w pd) d -> b (d pd) (h ph) (w pw)', h=h // self.ph, w=w // self.pw, pd=D,
...
class CSViT3d(nn.Module):
    def __init__(self, image_size, in_ch: int, num_classes: int = 2,  # dataset related
                 ...
                 dsconv: bool = False, attn_type: Literal['normal', 'mobile', 'parr_normal', 'parr_mobile'] = 'normal',
                 # module type
        ...
        if block_setting is None:
            block_setting = [
                ...
                ['c', 64, 3, 2, groups, 3, 0],
                # downsample + 3 conv # [B, g*C, D, L/2(256), W/2(256)] -> [B, g*C, D, L/4(128), W/4(128)]
                ['c', 96, 3, 2, groups, 1, 0],
                # downsample + conv # [B, g*C, D, L/4(128), W/4(128)] -> [B, g*C, D, L/8(64), W/8(64)]
                ['c', 160, 3, 2, groups, 1, 0],
                # downsample + conv # [B, g*C, D, L/8(64), W/8(64)] -> [B, g*C, D, L/16(32), W/16(32)]
                ['t', 160, 3, pw, groups, 3, 640],
                # vit # [B, g*C, D, L/16(32), W/16(32)] -> [B, g*C, D, L/16(32), W/16(32)]
            ]
        if dsconv:
...
def make_csv3dmodel(img_2dsize, inch, num_classes=2,
    ...
    block_setting = [
        ...
        ['c', 96, 3, 2, groups, 1, 0],
        # downsample + conv # [B, g*C, D, L/4(128), W/4(128)] -> [B, g*C, D, L/8(64), W/8(64)]
        ['c', tchannel, 3, 2, groups, 1, 0],
        # downsample + conv # [B, g*C, D, L/8(64), W/8(64)] -> [B, g*C, D, L/16(32), W/16(32)]
        ['t', tchannel, 3, patch_size[0], groups, 3, 640],
        # vit # [B, g*C, D, L/16(32), W/16(32)] -> [B, g*C, D, L/16(32), W/16(32)]
    ]
