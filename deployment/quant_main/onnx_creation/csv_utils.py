...
def conv_nxn_bn_group(inp: int, oup: int, kernal_size: Union[int, tuple] = 3, stride: Union[int, tuple] = 1,
                      groups: int = 4):
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # [B, (ph*pw), heads, L/ph*W/pw, dim_head] matmul [B, (ph*pw), heads, dim_head, L/ph*W/pw] * self.scale
        # = [B, (ph*pw), heads, L/ph*W/pw, L/ph*W/pw]
...
class ViTBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if 'mobile' in self.attn_type:
            ...
            # B, C, L, W --> B, L/ph*W/pw, (ph*pw*C)
            x = self.transformer(x)
            x = rearrange(x, 'b (h w) (ph pw d) -> b d (h ph) (w pw)', h=h // self.ph, w=w // self.pw, ph=self.ph,
                          pw=self.pw)
